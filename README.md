...
**На новом ПК один раз:** `repair_venv.bat` (если ошибка `No Python at C:\Users\...`).

`.venv` привязан к Python того компьютера, где его создали. Пути внутри не переносятся.

| Копировать | Зачем |
|------------|--------|
| `models/` | **Обязательно** — локальная модель |
| `server/`, `browser/`, `*.py`, `*.bat` | Код и запуск |
| `.venv/` | Можно, но на новом ПК всё равно `repair_venv.bat` |

Краткая инструкция: [USB_README.txt](USB_README.txt)
